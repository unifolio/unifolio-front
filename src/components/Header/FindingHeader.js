    ...
    <FindingHeaderPosition>
      <FindingHeaderLayout>
        <HeaderItem 
          ...
          current={current} value={"waiting-people"}
        >
          출자 대기중인 출자자
        </HeaderItem>
        <HeaderItem onClick={() => handleClickHeaderItem("waiting-unions")} 
          current={current} value={"waiting-unions"}
        >
          결성 대기중인 개인투자조합
        </HeaderItem>
      </FindingHeaderLayout>
...
const HeaderItem = styled.div`
  ...
  ${({current, value}) => current === value && css`
