...
const BusinessUnionCreate05 = ({
    ...
    { title: '회차', dataIndex: '회차', key: '회차' },
    {
      ...
      render: (text, record, index) => {
        ...
        return (
          <Input
            type={'number'}
            name={`invest_year_${record.key}`}
            value={invest_year[record.key]}
            onChange={onChangeInvestFieldInput}
          />
        );
      },
    },
    {
      ...
      render: (text, record, index) => {
        ...
        return (
          <Input
            type={'number'}
            name={`recovery_year_${record.key}`}
            value={recovery_year[record.key]}
            onChange={onChangeRecoverFieldInput}
          />
        );
      },
    },
  ...
  const onRowAddButtonClick = () => {
    ...
    const newRow = {
      key: rowsData.length + 1,
      회차: `${rowsData.length + 1}차 연도`,
    };
      ...
      <section>
        <div className='row'>
          <div className='column'>
            ...
            <Button onClick={onRowAddButtonClick} size='large'>
              {' '}
              연도 추가하기{' '}
            </Button>
          </div>
