...
const DefaultInfoBusiness = ({ user, handleSubmit }) => {
  ...
  const informations = [
    {
      labelKr: "법인 이름",
      labelEn: "corporate_name",
      value: user.corporate_name,
    },
    { labelKr: "이메일", labelEn: "email", value: user.email },
    {
      labelKr: "사업자 등록번호",
      labelEn: "corporate_registration",
      value: user.corporate_registration,
    },
    {
      labelKr: "법인 등록번호",
      labelEn: "company_registration_number",
      value: user.company_registration_number,
    },
    // {labelKr: "연락처", labelEn: "phone_number", value:user.phone_number ?? "연락처 입력이 필요합니다"},
    {
      labelKr: "우편번호",
      labelEn: "address_postcode_business",
      value: user.address_postcode_business,
    },
    {
      labelKr: "사무실 주소",
      labelEn: "address_business",
      value: user.address_business,
    },
    {
      labelKr: "사무실 상세 주소",
      labelEn: "address_detail_business",
      value: user.address_detail_business,
    },
  ...
  function callbackCompleteSearchPostcodeProcess(data) {
    setFormData({
      ...formData,
      address_business: data.address,
      address_postcode_business: data.zonecode,
    });
  }
  ...
  return (
    <DefaultInfoLayout>
      <DefaultInfoHeader>
        ...
        <DefaultInfoModifyButton
          isModifing={isModifing}
          onClick={handleClickModifyButton}
        />
        {isModifing && (
          <DefaultInfoModifyCancelButton onClick={() => setIsModifing(false)} />
        )}
      </DefaultInfoHeader>
        ...
        <InfoPresenter>
          {informations.map(({ labelKr, labelEn, value }) => (
            <InfoRow key={`${labelEn}-modify`}>
              ...
              {value ? (
                <InfoContents> {value} </InfoContents>
              ) : (
                <InfoContents color={"red"}>
                  {`${labelKr} 입력이 필요합니다.`}
                </InfoContents>
              )}
            </InfoRow>
